...
const FormPostagem = () => {
    ...
    const temaServices = new TemaService();
    ...
    async function buscarTemas() {
        try {
            await temaServices.buscarTemas('/temas', setTemas, {
                headers: { Authorization: token }
            })
        } catch (error: any) {
            if (error.toString().includes('403')) {
    ...
    async function gerarNovaPostagem(e: ChangeEvent<HTMLFormElement>) {
        ...
        if (id !== undefined) {
            try {
                await postagemServices.atualizarPostagem(`/postagens`, postagem, setPostagem, {
                    headers: {
                        Authorization: token,
                    },
                });
            ...
            try {
                await postagemServices.cadastrarPostagem(`/postagens`, postagem, setPostagem, {
                    headers: {
                        Authorization: token,
                    },
                })
